...
class World:
    ...
    def checkCollision(self):
        ...
        for obs in self.obstacles:
            if obs.intersectLine(pt1, pt2):
                return True
            if obs.intersectLine(pt2, pt3):
                return True
            if obs.intersectLine(pt3, pt4):
                return True
            if obs.intersectLine(pt4, pt1):
                return True
    ...
    def simulate(self, velR, velL, timestep):
        ...
        if self.proxL is not None and self.proxL < self.robot.minRange:
            self.proxL = None

        self.proxR, self.pointR = self.getRightDist()
        if self.proxR is not None and self.proxR < self.robot.minRange:
            self.proxR = None
